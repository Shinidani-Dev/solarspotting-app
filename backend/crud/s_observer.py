...
async def create_observer(db: AsyncSession, obs: ObserverCreate) -> Tuple[Optional[Observer], Optional[str]]:
    """
    Create a new observer in the database
    Args:
        db: the database
        observer: the schema to follow
    Returns:
        Tuple containing:
        - The created Observer object or None if failed
        - Error message string if failed or Non if successful
    """
    query = select(Observer).where(Observer.user_id == obs.user_id)
    result = await db.execute(query)
    if result.scalars().first():
        return None, f"Observer with user_id {obs.user_id} already exists"
    ...
    try:
        ...
        logger.info(f"executing statemet: {stmt}", module="crud/observer")
        ...
        await db.rollback()
        logger.error(f"[Obserer Creation Error] {e}", module="crud/observer", user_id=obs.user_id)
...
async def get_observer_by_user_id(db: AsyncSession, user_id: int) -> Optional[Observer]:
    """
    Get Observer by USER ID
    """
    query = select(Observer).where(Observer.user_id == user_id)
    logger.info(f"executing query: {query}", module="crud/observer")
    result = await db.execute(query)
    return result.scalars().first()
...
async def delete_observer(db: AsyncSession, obs_id: int) -> bool:
    ...
    logger.info(f"executing query: {stmt}", module="crud/observer")
