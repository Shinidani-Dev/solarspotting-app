        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-slate-400 mt-1">
              Modell trainieren und überwachen
            </p>
          </div>
            ...
            {trainingStatus?.is_running ? (
              <div className="space-y-3">
                <div className="flex items-center gap-2 text-amber-400">
                  ...
                  <span className="font-medium">Training läuft...</span>
                </div>
                ...
                <div className="text-sm text-slate-400">
                  <p>Epoche: {trainingStatus.current_epoch} / {trainingStatus.total_epochs}</p>
                  <p>Fortschritt: {trainingStatus.progress_percent?.toFixed(1)}%</p>
                  {trainingStatus.message && (
                    <p className="mt-1 text-slate-500">{trainingStatus.message}</p>
                  )}
                </div>
                ...
                {trainingStatus.metrics && Object.keys(trainingStatus.metrics).length > 0 && (
                  <div className="mt-3 pt-3 border-t border-slate-700">
                    <p className="text-xs text-slate-500 mb-1">Aktuelle Metriken:</p>
                    <div className="grid grid-cols-2 gap-2 text-xs">
                      {trainingStatus.metrics.box_loss && (
                        <div>
                          <span className="text-slate-500">Box Loss:</span>
                          <span className="text-slate-300 ml-1">{trainingStatus.metrics.box_loss}</span>
                        </div>
                      )}
                      {trainingStatus.metrics.cls_loss && (
                        <div>
                          <span className="text-slate-500">Cls Loss:</span>
                          <span className="text-slate-300 ml-1">{trainingStatus.metrics.cls_loss}</span>
                        </div>
                      )}
                    </div>
                  </div>
              ...
              <div className="space-y-2">
                <div className={`flex items-center gap-2 ${
                  ...
                  {trainingStatus?.status === 'completed' && <CheckCircle size={18} />}
                  {trainingStatus?.status === 'failed' && <AlertCircle size={18} />}
                  <span className="capitalize">{trainingStatus?.status || 'Bereit'}</span>
                </div>
                
                {trainingStatus?.finished_at && (
                  <p className="text-xs text-slate-500">
                    Beendet: {new Date(trainingStatus.finished_at).toLocaleString('de-CH')}
                  </p>
                )}
                
                {trainingStatus?.status === 'failed' && trainingStatus?.message && (
                  <p className="text-xs text-red-400 mt-2">
                    Fehler: {trainingStatus.message}
                  </p>
                )}
              </div>
            )}
                ...
                <tbody>
                  {datasetStats.archived_datasets.map((archive, idx) => (
                    <tr key={idx} className="border-b border-slate-700/50">
                      <td className="py-2 text-slate-300">{archive.filename}</td>
                      <td className="py-2 text-slate-400">{archive.size_mb} MB</td>
