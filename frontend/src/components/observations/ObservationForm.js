...
import { instrumentService, observationService, dayDataService, groupDataService } from '@/api/apiServices';
...
export default function ObservationForm({ observation = null, dayData = null, groupData = null, isEdit = false }) {
  ...
  useEffect(() => {
    if (instruments?.length > 0 && !formData.instrument_id) {
      ...
      const activeInstrument = instruments.find(instrument => instrument.in_use === true);
  ...
  useEffect(() => {
    if (isEdit && dayData) {
      setDayDataState({
        ...
        d_date: dayData.d_date ? new Date(dayData.d_date).toISOString().split('T')[0] : getTodayDate(),
  ...
  useEffect(() => {
    if (isEdit && groupData?.length > 0) {
      setGroupDataState(groupData.map(group => ({
        id: group.id,
        g_code: group.g_code,
        g_date: group.g_date ? new Date(group.g_date).toISOString().split('T')[0] : getTodayDate(),
  ...
  const validateForm = () => {
    ...
    if (totalGroups > 0 && groupDataState.length !== totalGroups) {
      newErrors.totalGroups = 'The number of group entries does not match the total groups specified';
    }
  ...
  const uploadFiles = async () => {
    ...
    if (dailyProtocol) {
      try {
        const result = await observationService.uploadFile(dailyProtocol, 'protocol');
        if (result && result.filePath) {
    ...
    try {
      ...
      const submissionData = {
        ...
        group_data: groupDataState.map(group => ({
          ...
          g_ut: group.g_ut !== null && group.g_ut !== '' ? parseInt(group.g_ut, 10) : null,
          g_q: group.g_q !== null && group.g_q !== '' ? parseInt(group.g_q, 10) : null,
          g_nr: group.g_nr !== null && group.g_nr !== '' ? parseInt(group.g_nr, 10) : null,
          g_f: group.g_f !== null && group.g_f !== '' ? parseInt(group.g_f, 10) : null,
          g_p: group.g_p !== null && group.g_p !== '' ? parseInt(group.g_p, 10) : null,
          g_s: group.g_s !== null && group.g_s !== '' ? parseInt(group.g_s, 10) : null,
          g_sector: group.g_sector !== null && group.g_sector !== '' ? parseInt(group.g_sector, 10) : null,
          g_a: group.g_a !== null && group.g_a !== '' ? parseInt(group.g_a, 10) : null,
        }))
      };
  ...
  const handleUpdate = async () => {
    try {
      ...
      queryClient.invalidateQueries({ queryKey: ['observations'] });
      queryClient.invalidateQueries({ queryKey: ['detailedObservation', observation.id] });
  ...
  // Loading state
  const isLoading = createMutation.isPending || loadingInstruments || isSubmitting || 
                   updateObservationMutation.isPending || updateDayDataMutation.isPending || 
                   createGroupDataMutation.isPending || updateGroupDataMutation.isPending || 
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      {errors.submit && (
        <div className="p-4 mb-4 border-l-4 border-red-500 rounded-md bg-red-500/10">
          <p className="text-red-400">
            {typeof errors.submit === 'string' ? errors.submit : 'An error occurred during submission'}
          </p>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
        ...
        <Card>
          <h3 className="mb-3 text-lg font-semibold text-amber-400">Daily Protocol (PDF)</h3>
          ...
          <div className="mb-4">
            ...
            {errors.dailyProtocol && <p className="error-msg">{errors.dailyProtocol}</p>}
          </div>
      ...
      <Card>
        <h3 className="mb-4 text-lg font-semibold text-amber-400">Observation Details</h3>
            ...
            {instruments?.length === 0 ? (
              <div className="p-4 border-l-4 border-yellow-500 rounded-md bg-yellow-900/20">
                <p className="text-sm text-yellow-400">
                  You do not have any instruments yet. Please create an instrument first.
                </p>
                <Button
                  type="button"
                  variant="outline"
              ...
              <>
                ...
                {errors.instrument_id && <p className="error-msg">{errors.instrument_id}</p>}
              </>
            )}
          </div>
      ...
      <Card>
        <h3 className="mb-4 text-lg font-semibold text-amber-400">Observation Summary</h3>
          ...
          <FormField
            ...
            onChange={(e) => {
              ...
              if (newValue > groupDataState.length) {
                ...
                if (isEdit) {
                  // Mark removed groups for deletion if they have an ID
                  const groupsToRemove = groupDataState.slice(newValue);
                  const idsToRemove = groupsToRemove
                    .filter(group => group.id)
                    .map(group => group.id);
                  
                  if (idsToRemove.length > 0) {
                    setDeletedGroupIds(prev => [...prev, ...idsToRemove]);
                  }
                }
                
                // Remove excess groups from state
                setGroupDataState(groupDataState.slice(0, newValue));
              }
            }}
            ...
            <Card key={group.id || index}>
              ...
              <div className="grid grid-cols-2 gap-3 md:grid-cols-3 lg:grid-cols-4">
                <FormField
                  ...
                  type="number"
                  value={group.g_code || ''}
                  onChange={(e) => handleGroupDataChange(index, 'g_code', e.target.value)}
                />
                ...
                <FormField
                  ...
                  onChange={(e) => handleGroupDataChange(index, 'g_date', e.target.value)}
                />
                ...
                <FormField
                  ...
                  type="number"
                  value={group.g_ut || ''}
                  onChange={(e) => handleGroupDataChange(index, 'g_ut', e.target.value)}
                />
                ...
                <FormField
                  ...
                  type="number"
                  value={group.g_q || ''}
                  onChange={(e) => handleGroupDataChange(index, 'g_q', e.target.value)}
                />
                ...
                <FormField
                  ...
                  type="number"
                  value={group.g_nr || ''}
                  onChange={(e) => handleGroupDataChange(index, 'g_nr', e.target.value)}
                />
                ...
                <FormField
                  ...
                  type="number"
                  value={group.g_f || ''}
                  onChange={(e) => handleGroupDataChange(index, 'g_f', e.target.value)}
                />
                ...
                <FormField
                  ...
                  maxLength="3"
                  value={group.g_zpd || ''}
                  onChange={(e) => handleGroupDataChange(index, 'g_zpd', e.target.value)}
                />
                ...
                <FormField
                  ...
                  type="number"
                  value={group.g_p || ''}
                  onChange={(e) => handleGroupDataChange(index, 'g_p', e.target.value)}
                />
                ...
                <FormField
                  ...
                  type="number"
                  value={group.g_s || ''}
                  onChange={(e) => handleGroupDataChange(index, 'g_s', e.target.value)}
                />
                ...
                <FormField
                  ...
                  onChange={(e) => handleGroupDataChange(index, 'g_sector', e.target.value)}
                />
                ...
                <FormField
                  ...
                  type="number"
                  value={group.g_a || ''}
                  onChange={(e) => handleGroupDataChange(index, 'g_a', e.target.value)}
                />
                ...
                <FormField
                  ...
                  maxLength="6"
                  value={group.g_pos || ''}
                  onChange={(e) => handleGroupDataChange(index, 'g_pos', e.target.value)}
                />
              </div>
            </Card>
          ))}
      ...
      <Card>
        <div className="pb-4 mb-4 border-b border-slate-700">
          <h3 className="text-xl font-bold text-amber-400">Day Data (Auto-Calculated)</h3>
          <p className="mt-1 text-sm text-slate-400">
            This data is automatically calculated based on the group data you provide.
          </p>
        </div>
