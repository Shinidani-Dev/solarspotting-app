...
export const observationService = {
  ...
  updatePublicStatus: async (id, isPublic) => {
    const response = await apiClient.put(`/observations/${id}/publicize`, {
      is_public: isPublic,
    });
    return response.data;
  },
  ...
  updateVerificationStatus: async (id, isVerified) => {
    const response = await apiClient.put(`/observations/${id}/verify`, {
      verified: isVerified,
    });
    return response.data;
  },
  ...
  getDetailedObservation: async (id) => {
    const response = await apiClient.get(`/observation/detailed/${id}`);
    return response.data;
  },

  // Create a complete observation with day data and group data
  createDetailedObservation: async (data) => {
    const response = await apiClient.post("/observation/detailed", data);
    return response.data;
  },

  // Update a complete observation with day data and group data
  updateDetailedObservation: async (id, data) => {
...
export const dayDataService = {
  ...
  getDayDataByObservation: async (observationId) => {
    const response = await apiClient.get(
      `/day-data/observation/${observationId}`
    );
    return response.data;
  },
...
export const groupDataService = {
  ...
  getGroupDataByObservation: async (observationId) => {
    const response = await apiClient.get(
      `/group-data/observation/${observationId}`
    );
    return response.data;
  },
